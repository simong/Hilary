...
/**
 * Brings a Sakai Documents' navigation structure up to date.
 *
 * @param {Context}    restCtx         Standard REST Context object that contains the current tenant URL and the current user credentials
 * @param {String}     contentId       The ID of the Sakai Document we're updating a page for
 * @param {Object}     structure       The structured navigation object to store for the Sakai Document
 * @param {Function}   callback        Standard callback after the function completes
 * @param {Object}     callback.err    Error returned in the callback, if any
 */
var updateSakaiDocStructure = module.exports.updateSakaiDocStructure = function(restCtx, contentId, structure, callback) {
    var params = {
        'structure': structure
    };
    RestUtil.RestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/pages/structure', 'POST', params, callback);
};

/**
 * Creates a new page inside of a Sakai document and adds it to the navigation structure.
 *
 * @param {Context}    restCtx         Standard REST Context object that contains the current tenant URL and the current user credentials
 * @param {String}     contentId       The ID of the Sakai Document we're creating a new page for
 * @param {String}     title           The title of the page to be created
 * @param {Function}   callback        Standard callback after the function completes
 * @param {Object}     callback.err    Error returned in the callback, if any
 */
var createPage = module.exports.createPage = function(restCtx, contentId, title, callback) {
    var params = {
        'title': title
    };
    RestUtil.RestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/pages', 'POST', params, callback);
};

/**
 * Gets a page in a Sakai Document and returns the content.
 *
 * @param {Context}    restCtx         Standard REST Context object that contains the current tenant URL and the current user credentials
 * @param {String}     contentId       The ID of the Sakai Document we're getting a page from
 * @param {String}     pageId          The ID of the page to get
 * @param {Function}   callback        Standard callback after the function completes
 * @param {Object}     callback.err    Error returned in the callback, if any
 */
var getPage = module.exports.getPage = function(restCtx, contentId, pageId, callback) {
    RestUtil.RestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/pages/' + RestUtil.encodeURIComponent(pageId), 'GET', null, callback);
};

/**
 * Saves a version of the page and saves the edits to the current version.
 *
 * @param {Context}    restCtx         Standard REST Context object that contains the current tenant URL and the current user credentials
 * @param {String}     contentId       The ID of the Sakai Document we're editing a page for
 * @param {String}     pageId          The ID of the page to edit
 * @param {Object}     page            Object containing all the information to apply an edit. TODO: Contents TBD
 * @param {Function}   callback        Standard callback after the function completes
 * @param {Object}     callback.err    Error returned in the callback, if any
 */
var editPage = module.exports.editPage = function(restCtx, contentId, pageId, page, callback) {
    var params = {
        'page': page
    };
    RestUtil.RestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/pages/' + RestUtil.encodeURIComponent(pageId), 'POST', params, callback);
};

/**
 * Deletes a page from the Sakai Document and navigation structure.
 *
 * @param {Context}    restCtx         Standard REST Context object that contains the current tenant URL and the current user credentials
 * @param {String}     contentId       The ID of the Sakai Document we're deleting a page from
 * @param {String}     pageId          The ID of the page to delete
 * @param {Function}   callback        Standard callback after the function completes
 * @param {Object}     callback.err    Error returned in the callback, if any
 */
var deletePage = module.exports.deletePage = function(restCtx, contentId, pageId, callback) {
    RestUtil.RestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/pages/' + RestUtil.encodeURIComponent(pageId), 'DELETE', null, callback);
};

/**
 * Exports a Sakai Document as a PDF or IMSCP package.
 *
 * @param {Context}    restCtx         Standard REST Context object that contains the current tenant URL and the current user credentials
 * @param {String}     contentId       The ID of the Sakai Document we're exporting
 * @param {Function}   callback        Standard callback after the function completes
 * @param {Object}     callback.err    Error returned in the callback, if any
 */
var exportSakaiDoc = module.exports.exportSakaiDoc = function(restCtx, contentId, callback) {
    RestUtil.RestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/export', 'GET', null, callback);
};

/**
 * Retrieves revisions for a Sakai Document.
 *
 * @param {Context}    restCtx         Standard REST Context object that contains the current tenant URL and the current user credentials
 * @param {String}     contentId       The ID of the Sakai Document we're getting revisions for a page from
 * @param {String}     pageId          The ID of the page to get revisions from
 * @param {Function}   callback        Standard callback after the function completes
 * @param {Object}     callback.err    Error returned in the callback, if any
 */
var getSakaiDocRevisions = module.exports.getSakaiDocRevisions = function(restCtx, contentId, pageId, callback) {
    RestUtil.RestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/pages/' + RestUtil.encodeURIComponent(pageId) + '/revisions', 'GET', null, callback);
};

/**
 * Creates a new version of the page and restores an older version back to the current version.
 *
 * @param {Context}    restCtx         Standard REST Context object that contains the current tenant URL and the current user credentials
 * @param {String}     contentId       The ID of the Sakai Document we're restoring a revision of a page for
 * @param {String}     pageId          The ID of the page to restore a revision for
 * @param {String}     revisionId      The ID of the revision to restore
 * @param {Function}   callback        Standard callback after the function completes
 * @param {Object}     callback.err    Error returned in the callback, if any
 */
var restoreSakaiDocRevision = module.exports.restoreSakaiDocRevision = function(restCtx, contentId, pageId, revisionId, callback) {
    RestUtil.RestRequest(restCtx, '/api/content/' + RestUtil.encodeURIComponent(contentId) + '/pages/' + RestUtil.encodeURIComponent(pageId) + '/revisions', 'POST', revisionId, callback);
};
