 ...
 *
 * @param  {Context}    ctx             Standard context object containing the current user and the current tenant
 * @param  {String}     userId          The username of the user who needs a password reset token
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var generateResetPasswordTokenByUsername = module.exports.generateResetPasswordTokenByUsername = function(ctx, username, callback){
    // Create the loginid object first, so it can be passed into the validation
    ...
    _getUserIdFromLoginId(loginId, function(err, userId) {
        ...
        return getResetPasswordSecretByUserId(ctx, userId, callback);
    });
};
 ...
 *
 * @param  {Context}    ctx             Standard context object containing the current user and the current tenant
 * @param  {String}     userId          The id of the user who needs a password reset token
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var generateResetPasswordTokenByUserId = module.exports.generateResetPasswordTokenByUserId = function(ctx, userId, callback) {
    var validator = new Validator();
    validator.check(userId, {'code': 400, 'msg': 'Please specify a user id'}).isUserId();
    _validateLoginIdForLookup(validator, loginId);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }

    // Get the user's object as we will try to send them an email
    PrincipalsDAO.getPrincipal(userId, function(err, user) {
        ...
        Cassandra.runQuery('UPDATE "AuthenticationLoginId" USING TTL 86400 SET "passwordResetToken" = ? WHERE "loginId" = ?', [token, _flattenLoginId(loginId)], function(err) {
            if (err) {
                log().error({'err': err}, 'Error in creating a reset password token');
                return callback(err);
            }

            // Generate the email content with the password token
            var emailData = {
                'tenant': ctx.tenant(),
                'user': user,
                'username': username,
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var resetPassword = module.exports.resetPassword = function(ctx, username, secret, password, callback) {
    // Create the loginid object first, so it can be passed into the validation
    var loginId = new LoginId(ctx.tenant().alias, AuthenticationConstants.providers.LOCAL, username);
    var validator = new Validator();
    validator.check(username, {'code': 400, 'msg': 'Please specify a username'}).notEmpty();
    validator.check(secret, {'code': 400, 'msg': 'Please specify a secret'}).notEmpty();
    validator.check(password, {'code': 400, 'msg': 'Please specify a password'}).notEmpty();
    _validateLoginIdForLookup(validator, loginId);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
    ...
    Cassandra.runQuery('SELECT "userId", "passwordResetToken" FROM "AuthenticationLoginId" WHERE "loginId" = ?', [_flattenLoginId(loginId)], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            // No user found with that login id
        ...
        // Get the password reset token column out of the row
        var passwordResetToken = rows[0].get('passwordResetToken');

        // If the secret column was not found or its value didn't match, something is wrong
        if (!passwordResetToken || passwordResetToken.value !== secret) {
        ...
        // Change the password
        return _changePassword(loginId, password, callback);
    });
};
