...
describe('Authentication', function() {
    ...
    describe('OAuth', function() {
        ...
        describe('Authorization Code Grant', function() {
            it('verify flow', function(callback) {
                TestsUtil.generateTestUsers(localAdminRestContext, 2, function(err, users, simong, nico) {
                    ...
                    RestAPI.OAuth.createClient(localAdminRestContext, simong.user.id, clientDisplayName, function(err, client) {
                        ...
                        // Fake a grant
                        RestAPI.OAuth.startAuthorizationTransaction(nico.restContext, client.id, 'http://my.app.com', function(err, data) {
                            assert.ok(!err);

                            // We should get a transaction ID that we can use to authorize the client
                            assert.ok(data.transactionID);

                            // We should also get the client information so the UI can display an appropriate message
                            assert.ok(data.client);
                            assert.equal(data.client.id, client.id);
                            assert.equal(data.client.displayName, client.displayName);
                            // The secret and user must not be returned
                            assert.ok(!data.client.secret);
                            assert.ok(!data.client.userId);

                            // Approve it
                            RestAPI.OAuth.grantAuthorization(nico.restContext, data.transactionID, function(err, data, response) {
                                assert.ok(!err);

                                // Assert that the user is redirected to the client app
                                assert.equal(response.statusCode, 302);
                                assert.ok(response.headers.location);
                                var parts = url.parse(response.headers.location);
                                var queryString = querystring.parse(parts.query);
                                assert.equal(parts.host, 'my.app.com');
                                assert.ok(queryString.code);

                                // Trade the authorization code for an access token
                                var oauth = new OAuth.OAuth2(client.id, client.secret, 'http://' + global.oaeTests.tenants.localhost.host, null, '/api/auth/oauth/v2/token',  null);
                                oauth.getOAuthAccessToken(queryString.code, {'grant_type':'authorization_code', 'redirect_uri': 'http://my.app.com'}, function (err, accessToken, refreshToken, results) {
                                    assert.ok(!err);

                                    // Assert we retrieved an access token
                                    assert.ok(accessToken);

                                    // Assert we're authenticated when using that access token
                                    var nicoOAuthRestContext = _generateOAuthRestContext(global.oaeTests.tenants.localhost.host, accessToken);
                                    RestAPI.User.getMe(nicoOAuthRestContext, function(err, data) {
                                        assert.ok(!err);
                                        assert.equal(data.id, nico.user.id);
                                        return callback();
                                    });
                                });
                            });
