...
var _createUser = function(ctx, user, callback) {
    PrincipalsDAO.createUser(user, function(err) {
        ...
        // If the provided email address hasn't been verified yet, we send it an email through
        // which the user can verify they really own that address
        var userToEmail = user;
        if (!user.email) {
            userToEmail = _.extend({}, user, {'email': user.emailChange});
        }
        sendEmailToken(ctx, userToEmail, function(err) {
            if (err) {
                return callback(err);
            }
        ...
        _getUserByEmailIfNecessary(profileFields.email, oldUser.email, function(err, user) {
            ...
            PrincipalsDAO.updatePrincipal(userId, profileFields, function(err) {
                ...
                if (profileFields.emailChange) {
                    // As we store the email in a special `emailChange` field, we need to overlay
                    // it in the user model
                    var userToEmail = _.extend({}, newUser);
                    userToEmail.email = profileFields.emailChange;

                    // Send the token through which the user can verify their email address
                    sendEmailToken(ctx, userToEmail, function(err) {
                        if (err) {
                            return callback(err);
                        }
...
var verifyEmail = module.exports.verifyEmail = function(ctx, email, token, callback) {
    ...
    PrincipalsDAO.getUserByEmailToken(token, function(err, user) {
        ...
        // TODO: Emit event
        return PrincipalsDAO.setEmailAddress(user, email, token, callback);
    });
};
