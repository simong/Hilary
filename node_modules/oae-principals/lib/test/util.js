...
var assertCreateUserSucceeds = module.exports.assertCreateUserSucceeds = function(restCtx, params, callback) {
    ...
    RestAPI.User.getMe(restCtx, function(err, me) {
        ...
        // Depending on whether we're an admin or not, we'll have to wait for the verification email
        var done = function() {
            nrOfCalls++;
            if (isAdmin && nrOfCalls === 1) {
                return callback(user);
            } else if (!isAdmin && nrOfCalls === 2) {
...
var assertUpdateUserSucceeds = module.exports.assertUpdateUserSucceeds = function(restCtx, userId, update, callback) {
    ...
    RestAPI.User.getUser(restCtx, userId, function(err, user) {
        assert.ok(!err);
        var isEmailChange = (update.email && update.email !== user.email);
        ...
        RestAPI.User.getMe(restCtx, function(err, me) {
            ...
            var token = null;
            var done = function() {
                if (!expectVerificationEmail) {
                    return callback(user);
                } else if (expectVerificationEmail && user && token) {
                    return callback(user, token);
                }
            };
            ...
            if (expectVerificationEmail) {
                whenVerificationEmailSent(update.email, function(_token) {
                    token = _token;
                    done();
                });
            }
...
var assertResendEmailTokenSucceeds = module.exports.assertResendEmailTokenSucceeds = function(restCtx, userId, callback) {
    ...
    RestAPI.User.getUser(restCtx, userId, function(err, user) {
        ...
        whenVerificationEmailSent(user.email, function(_token) {
            token = _token;
            done();
        });
