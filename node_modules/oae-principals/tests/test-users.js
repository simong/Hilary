        ...
        it('verify an email address cannot be re-used', function(callback) {
            ...
            TestsUtil.createTenantWithAdmin(tenantAlias, tenantHost, function(err, tenant, tenantAdminRestContext, tenantAdmin) {
                ...
                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, tenantAlias, {'oae-principals/recaptcha/enabled': false}, function(err) {
                    ...
                    RestAPI.User.createUser(tenantAdminRestContext, username1, 'password', 'Test User', email, {'visibility': 'public'}, function(err, createdUser) {
                        assert.ok(!err);

                        // The second user can't be created as the email address is already used
                        ...
                        RestAPI.User.createUser(anonymousRestContext, username2, 'password', 'Test User', email, {'visibility': 'public'}, function(err, createdUser) {
                            ...
                            // Sanity-check that even a tenant admin can't create the user
                            RestAPI.User.createUser(tenantAdminRestContext, username2, 'password', 'Test User', email, {'visibility': 'public'}, function(err, createdUser) {
                                assert.ok(err);
                                assert.strictEqual(err.code, 400);
                                return callback();
                            });
