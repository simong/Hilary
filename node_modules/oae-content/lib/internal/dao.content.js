 ...
 */
var createContent = module.exports.createContent = function(tenantAlias, createdBy, resourceSubType, displayName, description, visibility, additionalMembers, otherValues, revisionData, callback) {
    ...
    RevisionsDAO.createRevision(contentId, tenantAlias, createdBy, revisionData, function(err, revision) {
        if (err) {
            return callback(err);
        }
...
var _rowToContent = function(row) {
    ...
    var contentObj = new Content(hash.tenant, hash.contentId, hash.visibility, hash.displayName, hash.description, hash.resourceSubType, hash.createdBy, hash.created, hash.lastModified, hash.latestRevisionId);
