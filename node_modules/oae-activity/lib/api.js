 ...
 * @param  {Object}             options.description                                         An object containing more information about what this activity type is about and what the expected properties are
 * @param  {String}             options.description.msg                                     A human-readable string explaining when this activity is generated
 * @param  {Object}             options.description.entities                                An object containing the expected properties for this activitiy
 * @param  {String|String[]}    options.description.entities.actor                          A string or an array of strings outlining the expected activity entity types for the `actor` property of this activity
 * @param  {String|String[]}    options.description.entities.object                         A string or an array of strings outlining the expected activity entity types for the `object` property of this activity
 * @param  {String|String[]}    options.description.entities.target                         A string or an array of strings outlining the expected activity entity types for the `target` property of this activity
 ...
 * @param  {String}                 assocationDescription                       A string that accurately describes what this association returns
 ...
 */
var registerActivityEntityAssociation = module.exports.registerActivityEntityAssociation = function(activityEntityType, associationName, assocationDescription, associationFunction) {
    if (_.isFunction(assocationDescription)) {
        associationFunction = assocationDescription;
        assocationDescription = 'TODO';
    }

    ActivityRegistry.registerActivityEntityAssociation(new ActivityModel.EntityAssocation(activityEntityType, associationName, assocationDescription, associationFunction));
};
...
/**
 * Get information regarding what entities, assocations and activity types have been
 * registered with the system.
 *
 * @return {Object}     The object holding the information that is registered with the system
 */
var getInfo = module.exports.getInfo = function() {
    // The registerested entity types such as `user`, `group`, ..
    var entities = {};
    _.each(ActivityRegistry.getRegisteredActivityEntityTypes(), function(entity, entityName) {
        entities[entityName] = entity.description || 'TODO';
    });

    // The assocations per entity type (users: `self`, `followers`, ..)
    var assocations = {};
    _.each(ActivityRegistry.getRegisteredActivityEntityAssociations(), function(assocationsPerEntityType, entityType) {
        assocations[entityType] = [];
        _.each(assocationsPerEntityType, function(assocation, associationName) {
            assocations[entityType].push({'name': associationName, 'description': assocation.description});
        });
    });

    // The activity types themselves (`discussion-create`, `discussion-update`, ...)
    var registeredActivityTypes = ActivityRegistry.getRegisteredActivityTypes();

    return {
        'entities': entities,
        'types': registeredActivityTypes,
